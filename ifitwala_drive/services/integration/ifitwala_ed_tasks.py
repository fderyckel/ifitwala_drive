...
def validate_task_resource_finalize_context(upload_session_doc) -> dict[str, Any] | None:
	...
	for session_field, authoritative_field in field_map.items():
		if getattr(upload_session_doc, session_field, None) != authoritative[authoritative_field]:
			frappe.throw(
				_(
					"Upload session no longer matches the authoritative Task resource context for field '{0}'."
...
def run_task_post_finalize(upload_session_doc, created_file) -> dict[str, Any]:
	...
	file_size = getattr(created_file, "file_size", None) or frappe.db.get_value(
		"File", created_file.name, "file_size"
	)
...
def upload_task_resource_service(payload: dict[str, Any]) -> dict[str, Any]:
	...
	if provided_row_name:
		_assert_task_resource_row_exists(task_doc, _normalize_row_key(provided_row_name))

	authoritative = build_task_resource_upload_contract(task_doc, row_name=provided_row_name)
	row_key = authoritative["row_name"]
